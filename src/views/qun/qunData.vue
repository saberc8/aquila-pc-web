	...
	<div class="container">
		...
		<div class="content">
			...
			<div class="table-right">
				<h3>数据查看</h3>
				<el-table :data="qunTableData" :border="true" style="width: 100%">
					...
					<el-table-column label="数据录入时间" prop="createAt">
						<template #default="{ row }">
...
qunName.value = route.query.qunName

const getQunDetailFn = async (id) => {
	const res = await getQunDetail(id)
	console.log(res, '---')
	qunId.value = Number(id)
...
getQunDetailFn(route.params.id)
...
// 监听keep-alive离开生命周期
onDeactivated(() => {
	console.log('QunBindData deactivated')
})
onActivated(() => {
	console.log('QunBindData activated')
})
...
const getQunTypeListFn = async (params) => {
	const res = await getQunTypeList(params)
	console.log(res)
	if (res.code === 200) {
